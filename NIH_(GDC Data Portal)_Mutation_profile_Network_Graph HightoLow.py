#NIH (GDC Data Portal) Mutation Profile Network Graph Generator High to Low Mutation Count
#This module buildes the network graph from the case with the most mustations to the one with the fewest.
    ...
    for k1 in dict_list.keys():
        ...
        for k2 in dict_list.keys():
            if len(dict_list[k1]) <= len(dict_list[k2]):
                ...
                fract2 = len(set(dict_list[k1]) & set(dict_list[k2])) / len(dict_list[k1])
                if fract2 == fract1:
